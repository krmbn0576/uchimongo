	...
	function start(user) {
		...
		Game_Interpreter.prototype.pluginCommand = function(command, args) {
			...
			if (command.toLowerCase() === 'online') {
				...
				case 'to':
if (args[2] === 'news' && news) {
	news.set($gameVariables.value(+args[0]));
}
					var info = playerInfo();
...
var dispose = false;
news.on('value', function(data) {if (!dispose) {dispose = true;return;}
	if (data.val()) {
		$gameVariables.setValue(9, data.val());
	...
	Game_Avatar.prototype.moveSmooth = function(onlineData) {		
		...
		this.setMoveSpeed(onlineData.speed);
		this.setImage(onlineData.charaName, onlineData.charaIndex);
if (Math.abs(dx) + Math.abs(dy) > 5) {this.locate(onlineData.x, onlineData.y);this.setDirection(onlineData.direction);route.list.push({code: 0, parameters: []});this.forceMoveRoute(route);return;}
		if (dx !== 0) {
			var l = Math.abs(dx);
