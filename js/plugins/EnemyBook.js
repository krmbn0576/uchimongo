...
(function() {
    ...
    Game_Interpreter.prototype.pluginCommand = function(command, args) {
        ...
        if (command === 'EnemyBook') {
            ...
            case 'clear':
                ...
                break;
            case 'count':
                $gameSystem.countEnemyBook(Number(args[1]));
                break;
            }
        }
    };
    ...
    Game_System.prototype.countEnemyBook = function(val) {
        if (this._enemyBookFlags) {
            $gameVariables.setValue(val, this._enemyBookFlags.filter(function(flag) {return flag;}).length);
        }
    };
    ...
    Window_EnemyBookIndex.prototype.updateStatus = function() {
        if (this._statusWindow) {
            if (this.index() === this._list.length && (this.isOkTriggered() || this._touching)) this.processCancel();
    ...
    Window_EnemyBookIndex.prototype.drawItem = function(index) {
        ...
        this.drawText(index === this._list.length ? '図鑑を閉じる' : name, rect.x, rect.y, rect.width);
    };
